...
if [ $# -ne 2 ]; then
    echo "Pass desired size in MB, then pass 0 to build the disk image from scratch (Buildroot's output), or pass 1 to build the disk image from the existing RootFS directory"
    exit 1
fi
...
cp -a $RISCV/sysroot/usr/lib $OUTPUT_DIR/RootFS/usr/lib
dd if=/dev/zero of=riscv_disk bs=1M count=$1
